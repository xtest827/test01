...
# Plotting function for training visualization
...
# Neural Network for Genetic Algorithm and DQN
...
# Bird class with enhanced animation
class Bird(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, sounds, use_ga=False):
        super().__init__()
        self.sounds = sounds
        self.vel = 0
        self.flying = True
...
class Game:
    def __init__(self, width=WIN_WIDTH, height=WIN_HEIGHT):
        ...
        self.parameter_len = (Bird.input_size * Bird.hidden_size +
                              ...
                              Bird.output_size)
        self.nS = 3  # For DQN, use continuous state space
        self.nA = 2
    ...
    def reset(self, next_generation=None):
        ...
        if self.use_ga:
            for i in range(self.generation_size):
                ...
                bird = Bird(100, self.height // 2 + bird_height, self.sounds, use_ga=True)
    ...
    def draw(self):
        ...
        if (self.use_ga and not self.bird_group) or (
                not self.use_ga and self.bird_group and self.bird_group.sprites()[0].failed and
                self.bird_group.sprites()[0].touch_ground()):
    ...
    async def play_step(self, action=None):
        ...
        for event in pygame.event.get():
            ...
            if ((self.use_ga and not self.bird_group) or
                (not self.use_ga and self.bird_group and self.bird_group.sprites()[0].failed and
                 self.bird_group.sprites()[0].touch_ground())) and self.button.pressed(event):
                self.reset()
    ...
    def reproduce(self):
        ...
        if not self.game.weights or not self.game.fitness:
            ...
            for _ in range(self.game.generation_size):
                bird = Bird(100, self.game.height // 2, self.game.sounds, use_ga=True)

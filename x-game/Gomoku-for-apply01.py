...
class Game:
    ...
    def play_human_vs_ai(self, human_player, ai_player, start_player=1):
        ...
        while True:
            ...
            if not game_end:
                ...
                if self.board.current_player == human_player:
                    ...
                    self.message_area.draw(self.surface, "AI Thinking...", self.TextSize)
                pygame.display.flip()
                ...
                if self.board.current_player == human_player:
                    ...
                    move, _ = ai_player.get_action(self.board)
                    if move is not None:
                        self.board.do_move(move)
                        self.render_step(move)
    ...
    def play_ai_vs_ai(self, ai1, ai2, start_player=1, num_games=10):
        ...
        while game_count < num_games:
            clock.tick(60)
            if not game_end:
                self.message_area.draw(self.surface, f"Game {game_count + 1}: Player {self.board.current_player}'s turn", self.TextSize)
                pygame.display.flip()
                ...
                if has_winner:
                    game_end = True
                    self.message_area.draw(self.surface, f"Player {winner} wins!", self.TextSize)
                elif not self.board.availables():
                    game_end = True
                    self.message_area.draw(self.surface, "Draw!", self.TextSize)
                pygame.display.flip()
...
if __name__ == '__main__':
    ...
    n = 5
    start_player = 1
    model_path = "gomoku_policy_net.pth"

    # Initialize game and policy network
    ...
    # Run human vs AI game (default)
    game.play_human_vs_ai(human_player=1, ai_player=ai2, start_player=start_player)

    # Uncomment to run AI vs AI instead

...
class Game:
    ...
    def __init__(self, Width=800, Height=600):
        ...
        self.right_paddle = Paddle(self.surface, self.Win_width - 10 - self.paddle_width,
                                   self.Win_height // 2 - self.paddle_height // 2, self.paddle_width,
                                   self.paddle_height)
        self.left_score = 0
        self.right_score = 0
    ...
    def draw(self):
        ...
        left_score_text = self.font.render(f'{self.left_score}', True, self.WHITE)
        right_score_text = self.font.render(f'{self.right_score}', True, self.WHITE)
        self.surface.blit(left_score_text, (self.Win_width // 4 - left_score_text.get_width() // 2, 20))
        self.surface.blit(right_score_text, (self.Win_width * (3 / 4) - right_score_text.get_width() // 2, 20))
    ...
    def handle_collision(self):
        ...
        if self.ball.rect.left < 0:
            ...
            self.left_score += 1  # Fixed typo: left_Score to left_score
            self.ball.reset()
    ...
    def handle_paddle_movement(self):
        ...
        if keys[pygame.K_s] and self.left_paddle.rect.bottom + self.left_paddle.VEL <= self.Win_height:
            self.left_paddle.move(up=False)
        if keys[pygame.K_UP] and self.right_paddle.rect.top - self.right_paddle.VEL >= 0:
            self.right_paddle.move(up=True)
        if keys[
            pygame.K_DOWN] and self.right_paddle.rect.bottom + self.right_paddle.VEL <= self.Win_height:  # Fixed Win_width to Win_height
    ...
    def game_is_win(self):
        ...
        elif self.right_score >= self.win_score:  # Changed if to elif
            won = True
            win_text = "Right Player Won!"
        if won:
            text = self.font.render(win_text, 1, self.WHITE)
            self.surface.blit(text, (self.Win_width // 2 - text.get_width() // 2,
    ...
    def play_step(self):
        ...
        self.game_is_win()
        self.draw()
        pygame.time.Clock().tick(60)  # Fixed fpsClock to proper Clock object
        return game_over
